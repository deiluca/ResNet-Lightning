...
class ResNetClassifier(pl.LightningModule):
    ...
    def __init__(
        ...
        self.test_predictions, self.test_targets, self.test_embeddings = [], [], []

        # register forward hook to save embeddings
        self.resnet_model.avgpool.register_forward_hook(self.get_activation('avgpool'))
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.test_embeddings.extend(np.squeeze(self.activation['avgpool'].cpu().numpy()).tolist())
        ...
        if len(self.test_predictions) == self.df.shape[0]:
            ...
            self.df['embedding'] = self.test_embeddings
